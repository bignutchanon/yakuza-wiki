...
AdSense เคยตีตราเว็บนี้ว่า **"ต้องมีการดำเนินการ — เนื้อหาที่มีคุณค่าต่ำ"** เกณฑ์ที่ Google ประกาศไว้คือเนื้อหาต้อง original และเว็บต้องบอกได้ว่าใครเขียน ติดต่อได้ทางไหน (E-E-A-T) หน้าเว็บจึงถูกแก้ตามนี้ และ **กติกาด้านล่างต้องรักษาไว้ทุกครั้งที่เพิ่มหน้าใหม่**

- **ตัวตนผู้เขียน**: ใช้นามปากกา **BigNut** เท่านั้น ห้ามใส่ชื่อ-นามสกุลจริงลงหน้าเว็บทุกจุด · ทุกอย่างดึงจากค่าคงที่ `AUTHOR_NAME` / `CONTACT_EMAIL` (`yakuzathai.contact@gmail.com`) / `GITHUB_URL` ใน `src/lib/site.ts` อย่าพิมพ์ชื่อหรืออีเมลลงไปตรง ๆ
- **หน้า `/about`** (`src/app/about/page.tsx`) คือหน้าที่ตอบว่าใครทำเว็บ เนื้อหามาจากไหน ม็อดแจกอย่างไร มีโฆษณาเพราะอะไร และติดต่อได้ทางไหน — ห้ามลบ และถ้าข้อมูลผู้จัดทำเปลี่ยนต้องอัปเดตหน้านี้ด้วย
- **บทความทุกหน้าต้องมี `<Byline>`** (`src/components/Byline.tsx`) — ข่าว lore และหน้าบทใส่ครบแล้ว หน้าบทความชนิดใหม่ที่เพิ่มทีหลังต้องใส่ด้วย คู่กับ `articleJsonLd` ที่ประกาศ `author` เป็นโหนด Person (`PERSON_ID` ใน `src/lib/seo.ts`)
- **ความยาวหน้าบท**: บทสรุปเนื้อเรื่องทุกไฟล์ยาวอย่างน้อย ~900 ตัวอักษร (นับไม่รวมช่องว่าง) ปัจจุบันมัธยฐาน ~1,150 · บทใหม่ที่เขียนเพิ่มต้องไม่สั้นกว่านี้ ไม่งั้นสัดส่วนหน้าบางกลับมาอีก
- **หน้าบทมีบล็อกเสริมอัตโนมัติ 2 อัน**: "เควสเสริมที่เปิดในบทนี้" (มาจาก `substoriesForChapter()` ใน `src/lib/content.ts` ที่แกะคอลัมน์ "ปลดล็อก" ของตาราง substories — ตารางต้องคงรูปแบบ 5 คอลัมน์ `# | ชื่อ | สถานที่ | ปลดล็อก | สรุป` และเขียนบทเป็น "บทที่ N" ไม่งั้นบล็อกนี้หาย) และ "อ่านต่อในภาคนี้"
