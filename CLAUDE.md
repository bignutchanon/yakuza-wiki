...
- **ม็อดออกตัวแก้ (patch)**: (1) แก้ `mod.note` เป็น `'v<เวอร์ชัน> (<วันที่>) — <สรุปสั้น>'` (โชว์ต่อท้ายปุ่มดาวน์โหลด) (2) เขียนข่าว `src/content/news/YYYY-MM-DD-<game>-thai-mod-vXYZ.md` tag `ม็อดแปลไทย`
  ...
  ตัวอย่างล่าสุด: Y8 v1.0.2 (19 ส.ค. 2026) `2026-08-19-y8-thai-mod-v102.md`
...
- **Y8 (Like a Dragon: Infinite Wealth)** — released · v1.0.2 (19 ส.ค. 2026): Party Chat เป็นไทยแล้ว (เดิม EN/■ — ต้นตอ `sound_auth.bin` มีเพดานขนาด ~5.14 MB บีบให้ต่ำกว่าเพดาน ไม่ใช่ฟอนต์/คำแปล) + แก้เกมเด้งดาร์ตรีสอร์ต (รหัส `absolute`/`relative` ถูกแปล) + Bonds Bingo ตัดคำแนวตั้ง + tofu ป๊อปอัป + ชื่อเต็มตัวละครไทย · ลิงก์ Drive ใหม่ (`1aYAauqK…`) แทน v1.0.1 (`1gcaBpaw…`) · รายละเอียดใน `y8-infinite-wealth/HANDOFF.md` (หัวข้อ "RELEASE v1.0.2") · v1.0.1 (16 ส.ค.) = แก้ Miss Match ค้าง (reARMP)
