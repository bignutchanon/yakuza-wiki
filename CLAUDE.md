...
- **Judgment** — released · v1.0 (22 ส.ค. 2026) รุ่นแรก: แปลทั้งเกม 50,297 ประโยค / 159 ไฟล์ข้อความ · ฟอนต์ Sarabun วาดลงฟอนต์ในเกมโดยตรง (โหมด EN วาดทุกอย่างจาก bitmap grid `meta_ot_cond_book.dds` และตาราง advance อยู่ใน `db.judge.en.par → en/font.bin` ไม่ใช่ใน exe) · แจกเป็น `JudgmentThai-th-v1.0.zip` 8 MB พร้อม `install.bat` ที่หาโฟลเดอร์เกมจาก Steam เอง สำรองฟอนต์เดิมเป็น `.orig` และมี `uninstall.bat` · ผู้เล่นต้องตั้งภาษาข้อความในเกมเป็น English · **ยังไม่มีใครเล่นจบเกมด้วยม็อดนี้** — ค้างอยู่: ป้ายท่า EX สีเขียวตอนต่อสู้ตัวอักษรเพี้ยน, ป้าย MISSION/TIPS กับป้ายโต๊ะรูเล็ตเพิ่งแก้นาทีสุดท้ายยังไม่เคยเห็นบนจอจริง (ถ้ามีคนแจ้งว่าเกมค้างตอนขึ้นป้ายพวกนี้ ให้แจ้งกลับ `judgment-thai` ทันที) · รายละเอียดใน `judgment-thai/patch.md` + `HANDOFF.md` (รอบ 16)
- **Gaiden (Like a Dragon Gaiden)** — released · v1.0.2 (21 ส.ค. 2026): แก้โป๊กเกอร์/แบล็คแจ็คค้าง**ตอนเล่นครั้งแรก** — สาเหตุที่สอง คนละตัวกับ v1.0.1 · ต้นตอเดียวกับ Y8 v1.0.4 คือ `controller_guide.bin` (จอแนะนำปุ่มของมินิเกมทุกตัว) ที่ตัวสร้างไฟล์ประกอบใหม่แล้วเลย์เอาต์ไม่ตรงต้นฉบับ (ตัวแปลงของ Gaiden ยังเป็นฉบับก่อนแก้บั๊ก ต่างจากต้นฉบับ 136 เซลล์) · v1.0.2 พอร์ต `scripts/patch_text_inplace.py` จาก Y8 มาใช้ทั้งม็อด 130 ไฟล์ (`check_inplace_bins.py` 130/130 ผ่าน) · คำแปล/ฟอนต์ไม่เปลี่ยน · ผู้เล่นยืนยันในเกมแล้ว · ลิงก์ Drive ใหม่ (`1T-WNCex…`) แทน v1.0.1 (`1huUYI0b…`) · รายละเอียดใน `yakuza-gaiden/docs/patch.md` · v1.0.1 (18 ส.ค.) = สาเหตุแรก ตารางระบบถูกเขียนทับ + รหัสภายใน (`ok`/`absolute`/`stage`) ถูกแปลติด (`docs/wiki_update_v1.0.1.md`)
